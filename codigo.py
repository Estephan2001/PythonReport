...
for batch in test_ds.take(1):
    ...
    for i in range(len(pred_texts)):
        true_text = tf.strings.reduce_join(num_to_char(test_labels[i])).numpy().decode("utf-8")
        print(f"Predicted: {pred_texts[i]}, True: {true_text}")
...
# Guardar el modelo
model.save("handwriting_recognizer_model")

# Cargar el modelo
loaded_model = keras.models.load_model("handwriting_recognizer_model", custom_objects={"CTCLayer": CTCLayer})

# Ejemplo de predicción
def decode_batch_predictions(pred):
    input_len = np.ones(pred.shape[0]) * pred.shape[1]
    predictions = keras.backend.ctc_decode(pred, input_length=input_len, greedy=True)[0][0][:, :max_len]
    return [num_to_char(x) for x in predictions]

# Seleccionar un lote de imágenes de prueba
for batch in test_ds.take(1):
...
# Imprimir las imágenes y las etiquetas reales junto con las predicciones decodificadas
for i in range(len(batch_images)):
    print("----------------------------------------")
    plt.imshow(tf.squeeze(batch_images[i]), cmap="gray")
    plt.title(f"Real: {num_to_char(batch_labels[i].numpy())} \n Predicted: {decoded_preds[i]}")
    plt.axis("off")
    plt.show()
